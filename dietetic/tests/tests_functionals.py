...
"""class TestsFunctionalsResponsive(StaticLiveServerTestCase):
     class TestsFunctionalsResponsive :
    test the use of the responsive menu 

    fixtures = ['data.json']

    def setUp(self):
        self.browser = webdriver.Firefox()
        self.browser.set_window_size(360, 740)
        self.cursor = connection.cursor()
        self.user = get_user_model()
    ...
    def test_access_login_nav(self):
       check url returned
        self.browser.get(self.live_server_url + "/")
        time.sleep(40)

        self.create_user_starting_program()

        # tests access menu responsive
        access_list = [["fa-user-circle", "/"], ["login", "/"],
                       ["create_account", "/account/create_account/"],
                       ["fa-user-circle", "/account/create_account/"]]

        for elt in access_list:
            print(elt[0])
            self.browser.find_element_by_id(elt[0]).click()
            self.assertEqual(self.browser.current_url, self.live_server_url + elt[1])

        access_list = [["fa-bars", "/account/create_account/"],
                       ["fa-home", "/"], ["user", "/account/my_account/"],
                       ["fa-clipboard", "/dietetic/discussion_space/"],
                       ["fa-poll", "/dietetic/my_results/"],
                       ["fa-utensils", "/dietetic/program/"]]

        for elt in access_list:
            print(elt[0])
            self.browser.find_element_by_class_name(elt[0]).click()
            self.assertEqual(self.browser.current_url, self.live_server_url + elt[1])"""
